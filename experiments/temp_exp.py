...
class ClassifierExperiment(pl.LightningModule):
    ...
    def on_validation_epoch_end(self):
        ...
        if self.calculate_val_knn_accuracy:
            ...
            dm_metrics = self.calculate_distance_matrix_knn_acc(self.sub_sampled_val_phase_data)
            self.log("val_dm_knn_acc", dm_metrics, on_step=False, on_epoch=True)     
            self.val_phase_data = {"labels":[],"embeddings":[],"logits": []}
    ...
    def calculate_distance_matrix_knn_acc(self,test_data):
        ...
        latent_distance_matrix = dist_fn(test_data['embeddings']).numpy()
        labels = test_data['labels'].numpy()   
        knn_acc, _, _ ,_  = StaticDistanceMatrixMetricCalculator.evaluate_knn_classifier_from_distance_matrix(distance_matrix = latent_distance_matrix,labels=labels,k = 3)
        return knn_acc
    ...
    def configure_optimizers(self):
        ...
        print("Configuring Optimiziers")
        lr = self.lr["lr"] if isinstance(self.lr,dict) else self.lr
        if self.different_learning_rate_for_classifier:
            params = [
                
                {"params": filter(lambda p: p.requires_grad, self.classifier_model.classifier_head.parameters()), "lr": lr*3}
            ]
            if hasattr(self.classifier_model.backbone,"parameters"):
                params.append({"params": filter(lambda p: p.requires_grad, self.classifier_model.backbone.parameters()), "lr": lr})
        else:
        ...
        if isinstance(self.lr, dict):
            # Set up the learning rate scheduler based on the provided dictionary
