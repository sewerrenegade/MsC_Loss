...
def get_size_output_dim(size):
    return DINO_NETWORKS_INFOS[size]["out_dim"]

def get_dino_size(teacher_model_type: str) -> str:
    """Returns the size category of a DINO model based on its name."""
    teacher_model_type = teacher_model_type.lower()  # Normalize input to lowercase

    if any(size in teacher_model_type for size in ["giant", "g"]):
        return "giant"
    elif any(size in teacher_model_type for size in ["big", "b"]):
        return "big"
    elif any(size in teacher_model_type for size in ["large", "l"]):
        return "large"
    elif any(size in teacher_model_type for size in ["small", "s"]):
        return "small"
    else:
...
def get_dino_w_resize_and_model_nb_of_params(size = "small",target_data = "single_cell"):
    ...
    return resize_and_encode, model_nb_params, get_size_output_dim(size)
