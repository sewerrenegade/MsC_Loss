...
BEST_AE_ENCODER_MODEL = r"Connectivity_Distillation_Experiment/nhxncpji/checkpoints/best_model.ckpt"
...
def train_ae():
    ...
    trainer = pl.Trainer(max_epochs=100,callbacks=[checkpoint_callback],accelerator="gpu" if torch.cuda.is_available() else "cpu", devices=1,logger = logger,enable_progress_bar = not on_hpc)  # Use gpus=1 if GPU is available
...
def train_classifier():
    ...
    trainer = pl.Trainer(max_epochs=CLASSIFER_MAX_EPOCH,callbacks=[classifier_checkpoint_callback],accelerator="gpu" if torch.cuda.is_available() else "cpu", devices=1,logger = logger,enable_progress_bar = not on_hpc)  # Use gpus=1 if GPU is available
    trainer.fit(experiment, datamodule = datamodule)
    experiment.best_checkpoint_path = classifier_checkpoint_callback.best_model_path
    result = trainer.test(experiment,datamodule,ckpt_path=experiment.best_checkpoint_path)
    
def train_classifier_with_distillation(path_to_ae = BEST_AE_ENCODER_MODEL,distillation_loss_name = 'prop_topo'):
    datamodule = DualAcevedoImageDataModule(dataset_target="classifier")
    model = ImageClassifier(num_classes=NUM_OF_CLASSES)
    experiment = ClassifierExperiment(model= model,distillation_loss_name=distillation_loss_name,ae_encoder = ClassifierExperiment.get_ae_encoder_from_path(path_to_ae),distillation_weight=0.01)
    classifier_checkpoint_callback = ModelCheckpoint(
        monitor="val_accuracy", save_top_k=1, mode="max", filename="best_model"
            )
    trainer = pl.Trainer(max_epochs=CLASSIFER_MAX_EPOCH,callbacks=[classifier_checkpoint_callback],accelerator="gpu" if torch.cuda.is_available() else "cpu", devices=1,logger=logger, enable_progress_bar = not on_hpc)  # Use gpus=1 if GPU is available
...
if __name__ == "__main__":
    ...
    parser.add_argument(
    "--hpc", 
    action="store_true",  # Sets True if flag is provided, False otherwise
